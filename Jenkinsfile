pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                sh '''
                    docker compose stop gateway auth stock discovery mariadb || true; \
                    docker compose rm -f gateway auth stock discovery mariadb || true; \
                    docker compose up -d mariadb; \
                    sleep 15; \
                    docker compose up -d discovery; \
                    sleep 15; \
                    docker compose up -d auth gateway stock; \
                    sleep 30
                '''
            }
        }
            ...
            steps {
                script {
                    sh '''
                        ...
                        check_health "Stock" "zqksk-stock" "7004" || exit 1
